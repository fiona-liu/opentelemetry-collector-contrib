...
func TestParseMalformedCommandstat(t *testing.T) {
	tests := []struct{ name, commandstat string }{
		{"missing value", "calls=1,usec=2,usec_per_call=3.29,rejected_calls=4,failed_calls="},
		{"missing equals", "calls=1,usec=2,usec_per_call=3.29,rejected_calls=4,failed_calls"},
		{"unexpected key", "xyz,calls=1,usec=2"},
		{"no usable data", "foo"},
	}
